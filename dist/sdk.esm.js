...
var FACTORY_ADDRESS = '0x21424DDad619B5932A9245f7EDD3E60F8a684cf5';
var FACTORY_ADDRESS_MAP = (_FACTORY_ADDRESS_MAP = {}, _FACTORY_ADDRESS_MAP[ChainId.MAINNET] = FACTORY_ADDRESS, _FACTORY_ADDRESS_MAP[ChainId.TESTNET] = '0x21424DDad619B5932A9245f7EDD3E60F8a684cf5', _FACTORY_ADDRESS_MAP);
var INIT_CODE_HASH = '0xef6aba2147f4fc0aee4284683a2458441a2aad90980a048def5d6166cb656877';
var INIT_CODE_HASH_MAP = (_INIT_CODE_HASH_MAP = {}, _INIT_CODE_HASH_MAP[ChainId.MAINNET] = INIT_CODE_HASH, _INIT_CODE_HASH_MAP[ChainId.TESTNET] = '0xef6aba2147f4fc0aee4284683a2458441a2aad90980a048def5d6166cb656877', _INIT_CODE_HASH_MAP);
...
var composeKey = function composeKey(token0, token1, stable) {
  return token0.chainId + "-" + token0.address + "-" + token1.address + "-" + stable;
};

var Pair = /*#__PURE__*/function () {
  function Pair(tokenAmountA, tokenAmountB, stable) {
    var tokenAmounts = tokenAmountA.token.sortsBefore(tokenAmountB.token) // does safety checks
    ? [tokenAmountA, tokenAmountB] : [tokenAmountB, tokenAmountA];
    this.liquidityToken = new Token(tokenAmounts[0].token.chainId, Pair.getAddress(tokenAmounts[0].token, tokenAmounts[1].token, stable), 18, 'Cake-LP', 'Pancake LPs');
  ...
  Pair.getAddress = function getAddress(tokenA, tokenB, stable) {
    ...
    var key = composeKey(token0, token1, stable);

    if (true || ((_PAIR_ADDRESS_CACHE = PAIR_ADDRESS_CACHE) === null || _PAIR_ADDRESS_CACHE === void 0 ? void 0 : _PAIR_ADDRESS_CACHE[key]) === undefined) {
      var _extends2;

      PAIR_ADDRESS_CACHE = _extends({}, PAIR_ADDRESS_CACHE, (_extends2 = {}, _extends2[key] = getCreate2Address(FACTORY_ADDRESS_MAP[token0.chainId], keccak256(['bytes'], [pack(['address', 'address', 'bool'], [token0.address, token1.address, stable])]), INIT_CODE_HASH_MAP[token0.chainId]), _extends2));
    }
...
var Router = /*#__PURE__*/function () {
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    ...
    console.log("swapCallParameters===============>", trade, options)
    ...
    path.map((elt, i) => {
      if (i % 2 === 0) {
        const currPair = trade?.route.pairs?.find((pair) => (pair.tokenAmounts[0].token.address === path[i] && pair.tokenAmounts[1].token.address === path[i+1]) &&
        (pair.tokenAmounts[0].token.address === path[i+1] && pair.tokenAmounts[1].token.address === path[i])) 
        mroutes.push({
          from: path[i],
          to: path[i+1],
          stable: currPair ? currPair.stable : true
        })
      }
    })
    switch (trade.tradeType) {
      case TradeType.EXACT_INPUT:
        if (etherIn) {
          ...
          args = [amountOut, mroutes, to, deadline];
          value = amountIn;
        } else if (etherOut) {
          methodName = useFeeOnTransfer ? 'swapExactTokensForETHSupportingFeeOnTransferTokens' : 'swapExactTokensForETH'; // (uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)

          args = [amountIn, amountOut, mroutes, to, deadline];
          value = ZERO_HEX;
        } else {
      ...
      case TradeType.EXACT_OUTPUT:
        ...
        if (etherIn) {
          ...
          args = [amountOut, amountIn, mroutes, to, deadline];
          value = ZERO_HEX;
        } else {
...
var Fetcher = /*#__PURE__*/function () {
  ...
  Fetcher.fetchPairData = function fetchPairData(tokenA, tokenB, provider) {
    try {
      ...
      var address = Pair.getAddress(tokenA, tokenB, true);
